...
export const getCollection = catchAsync(async (req, res) => {
  const collection = await collectionService.getCollectionById(req.params.id, req.user)
  sendSuccess(res, HTTP.OK, 'Collection fetched', { collection })
})

export const retryAccount = catchAsync(async (req, res) => {
  const collection = await collectionService.retryAccountCreation(req.params.id)
